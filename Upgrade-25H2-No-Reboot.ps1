...
# Standalone no-automatic-restart x64 24H2 -> 25H2 deployment.
[CmdletBinding()]
param([switch]$Install,[switch]$AllowOfferBypass,[switch]$NoAutomaticRestart,[switch]$CheckOnly)
...
if ([Environment]::Is64BitOperatingSystem -and -not [Environment]::Is64BitProcess) {
    ...
    if($Install){$arguments+='-Install'}; if($AllowOfferBypass){$arguments+='-AllowOfferBypass'}; if($NoAutomaticRestart){$arguments+='-NoAutomaticRestart'}; if($CheckOnly){$arguments+='-CheckOnly'}
    & $native @arguments; exit $LASTEXITCODE
}
# This standalone worker is deliberately manual-restart-only.
$NoAutomaticRestart=$true
$Root='C:\ProgramData\Win11-25H2-NoReboot'
$Worker=Join-Path $Root 'Upgrade-25H2-No-Reboot.ps1'
$LogPath=Join-Path $Root 'Upgrade-25H2-No-Reboot.log'
$StatePath=Join-Path $Root 'no-reboot-state.json'
...
function Assert-OwnedTaskActions($tasks) {
    foreach($task in $tasks){
        ...
        foreach($action in $task.Actions){
            if(($action.Execute+' '+$action.Arguments) -notmatch '(?i)C:\\ProgramData\\Win11-25H2(?:-NoReboot)?\\(Upgrade-25H2(?:-No-Reboot)?\.ps1|Upgrade-24H2-to-25H2\.ps1|Win11-25H2(?:-No-Reboot)?-Launcher\.bat)(?=["\s]|$)'){
...
function Request-Reboot($reason,$boot) {
    ...
    $script:State.RestartBoot=''; Save-State 'AwaitingManualRestart' $reason
    Write-Log "MANUAL RESTART REQUIRED: $reason This standalone deployment contains no restart command or countdown."
}
function Get-LauncherText([bool]$noAutomaticRestart=$false) {
    $text=@'
@echo off
setlocal
set "ROOT=C:\ProgramData\Win11-25H2-NoReboot"
set "PS=%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe"
if exist "%SystemRoot%\Sysnative\WindowsPowerShell\v1.0\powershell.exe" set "PS=%SystemRoot%\Sysnative\WindowsPowerShell\v1.0\powershell.exe"
echo [%date% %time%] Direct25H2 launcher started.>>"%ROOT%\Launcher.log"
"%PS%" -NoLogo -NoProfile -NonInteractive -ExecutionPolicy Bypass -File "%ROOT%\Upgrade-25H2-No-Reboot.ps1" -AllowOfferBypass >>"%ROOT%\Launcher.log" 2>&1
set "RC=%ERRORLEVEL%"
echo [%date% %time%] Direct25H2 launcher finished with exit code %RC%.>>"%ROOT%\Launcher.log"
exit /b %RC%
'@
    if($noAutomaticRestart){$text=$text.Replace(' -AllowOfferBypass ',' -AllowOfferBypass -NoAutomaticRestart ')}
    return $text
}
function Install-Tasks([string]$sourceFile,[bool]$noAutomaticRestart=$false) {
    ...
    foreach($name in @('Upgrade-25H2-No-Reboot.ps1','Win11-25H2-No-Reboot-Launcher.bat')){
    ...
    $bat=Join-Path $Root 'Win11-25H2-No-Reboot-Launcher.bat'
    Write-LauncherFile $bat $noAutomaticRestart
    Protect-Path $Worker; Protect-Path $bat
    $action=New-ScheduledTaskAction -Execute "$env:SystemRoot\System32\cmd.exe" -Argument '/d /c C:\ProgramData\Win11-25H2-NoReboot\Win11-25H2-No-Reboot-Launcher.bat'
    ...
    $restartMode=if($noAutomaticRestart){'manual restart only'}else{'10-minute automatic restart'}
    Write-Log "Direct mode installed: SYSTEM BAT tasks at startup and every 3 hours; $restartMode. Offer and safeguard gating bypass authorized; signatures and applicability remain enforced."
}
function Write-LauncherFile($path,[bool]$noAutomaticRestart=$false) {
    [IO.File]::WriteAllText($path,((Get-LauncherText $noAutomaticRestart) -replace '\r?\n',"`r`n"),[Text.Encoding]::ASCII)
}
...
try {
    ...
    foreach($name in @('Upgrade-25H2-No-Reboot.log','Launcher.log','no-reboot-state.json')){$path=Join-Path $Root $name; Assert-Path $path; if(Test-Path -LiteralPath $path){Protect-Path $path}}
    ...
    $initialized=$true
    $restartMode=if($NoAutomaticRestart){'manual-only'}else{'automatic-10-minute'}
    Write-Log "Mode=direct-no-reboot; $($snapshot.Release) $($snapshot.Build).$($snapshot.UBR); running=$($snapshot.Running); bypass authorized; restart=$restartMode."
    if($Install){Install-Tasks $PSCommandPath ([bool]$NoAutomaticRestart); $startFirst=$true}
...
if($startFirst -and $exitCode -eq 0){Start-ScheduledTask -TaskPath '\' -TaskName 'Win11-25H2-Retry' -ErrorAction Stop; Write-Host 'Direct25H2 no-reboot SYSTEM tasks registered; first run requested. Read Upgrade-25H2-No-Reboot.log and no-reboot-state.json.'}
exit $exitCode
