...
function Remove-SetupLogArtifact([string]$Path) {
    $rootFull = [IO.Path]::GetFullPath($Root).TrimEnd('\') + '\'
    $pathFull = [IO.Path]::GetFullPath($Path)
    if (-not $pathFull.StartsWith($rootFull,[StringComparison]::OrdinalIgnoreCase)) {
        throw "Refusing to remove a Setup log artifact outside the deployment folder: $pathFull"
    }
...
function Invoke-CompatibilityScan([string]$Setup,[hashtable]$State) {
    Remove-SetupLogArtifact -Path $CompatLogs
...
function Invoke-InPlaceUpgrade([string]$Setup,[hashtable]$State) {
    Remove-SetupLogArtifact -Path $SetupLogs
